...
class BugAlgoClass(Node):
  ...
  def collecting_minibox(self):
    ...
    if abs(Eq) > 0.1 and (not self.flag4) :
      ...
      if abs(Ex) > 0.02 and abs(Ey) > 0.02 and (not self.flag5):
        ...
        if abs(Eq) > 0.1 and (not self.flag6):
          self.move_robot(0.0, w)
          ...
          self.flag6 = True
          Ex = self.bx - self.c_pose[0]
          Ey = self.by - self.c_pose[1]
          Eq = self.lastEq - self.c_pose[2]
          Eq = atan2(sin(Eq), cos(Eq))
          dis = sqrt(Ex**2 + Ey**2)

          
          if abs(Ex) > 0.015 and abs(Ey) > 0.015 and (not self.flag7):
            self.move_robot(dis*self.kl/2.0,Eq*self.kw*2.0)
            print("Corrigiendo b")
  ...
  def drop_box(self):
    if not self.flag3:
      ...
      self.cx = sin(self.c_pose[2]) * self.c + self.c_pose[0]
      self.cy = cos(self.c_pose[2]) * self.c + self.c_pose[1]
      self.flag3 = True
    ...
    if len(self.c_pose) > 0:
      match self.state:
        ...
        case "go_to_goal":
          ...
          if (self.c_pose[0] <= 0.7+ self.tol_target and self.c_pose[0] >= 0.7 - self.tol_target) and (self.c_pose[1] <= 0.0 + self.tol_target and self.c_pose[1] >= -0.0 - self.tol_target ) and (self.atTarget()) and (self.isMiniArucoAhead()):
            self.state = "collecting_minibox"
            self.get_logger().info("Collecting Mini Box.")
          elif (self.c_pose[0] >= -1.15+ self.tol_target and self.c_pose[0] <= -1.15 - self.tol_target) and (self.c_pose[1] <= 0.0 + self.tol_target and self.c_pose[1] >= -0.0 - self.tol_target ) and (self.atTarget()):
            self.state = "drop_box"
